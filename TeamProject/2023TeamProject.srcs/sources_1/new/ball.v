...
wire [20:0] meet_ball1_sqr, meet_ball2_sqr,meet_ball3_sqr,meet_ball4_sqr,meet_ball5_sqr;
wire meet_ball1,meet_ball2,meet_ball3,meet_ball4,meet_ball5;
reg [4:0] vx_c, vy_c;

assign ball_rad_sqr = (BALL_RAD+BALL_RAD)*(BALL_RAD+BALL_RAD);

assign meet_ball1_sqr = ((ball_x - ball_x_in1)*(ball_x - ball_x_in1)) + ((ball_y - ball_y_in1)*(ball_y - ball_y_in1));
...
always @(*) begin
    if(rst) vx_c <= 0;
        else if (meet_ball1) vx_c <= vx_in_1/2; 
        else if (meet_ball2) vx_c <= vx_in_2/2;
        else if (meet_ball3) vx_c <= vx_in_3/2;
        else if (meet_ball4) vx_c <= vx_in_4/2;
        else if (meet_ball5) vx_c <= vx_in_5/2;
        else begin end
end

always @(*) begin
    if(rst) vy_c <= -1;
        else if (meet_ball1) vy_c <= vy_in_1/2;
        else if (meet_ball2) vy_c <= vy_in_2/2;
        else if (meet_ball3) vy_c <= vy_in_3/2;
        else if (meet_ball4) vy_c <= vy_in_4/2;
        else if (meet_ball5) vy_c <= vy_in_5/2;
        else begin end
end
...
else if (meet_ball1 | meet_ball2 | meet_ball3 | meet_ball4 | meet_ball5) begin vx<=vx_c; vy<=vy_c; end
else if((speeddown)&((turn_st==0) | (turn_st==3))) begin if (vx<1) vx <=0; else if(vx > 0) vx <= vx/2;
...
always @(posedge clk or posedge rst) begin    //ball_x
    ...
    else if(((turn_st==0) | (turn_st==3))&(frame_tick))  // if your turn, move | after your turn, keep react.
